...
def main():
    ...
    room = channel.queue_declare(queue = 'ppd/WRoom/2')      # assina/publica - Sala de Espera
    channel.queue_bind(exchange='WRoom', queue=room.method.queue)

    channel.exchange_declare(exchange='Election', exchange_type='fanout')
    election = channel.queue_declare(queue = 'ppd/election/2')      # assina/publica - Eleção do presidente
    ...
    challenge = channel.queue_declare(queue = 'ppd/challenge/2')     # assina/publica - Desafio da transição atual
    ...
    seed = channel.queue_declare(queue = 'ppd/seed/2')     # assina/publica - Verificação da seed que resolve desafio
    channel.queue_bind(exchange='Seed', queue=seed.method.queue)

    channel.exchange_declare(exchange='Result', exchange_type='fanout')
    result = channel.queue_declare(queue = 'ppd/result/2')     # assina/publica - Lista de votação na seed que soluciona o desafio
    channel.queue_bind(exchange='Result', queue=result.method.queue)

    
    # Fila de Espera
    channel.basic_publish(exchange = 'WRoom', routing_key = '', body = str(id))
    channel.basic_consume(queue = 'ppd/WRoom/2' , on_message_callback = callback, auto_ack = True)
    
    # Eleição
    channel.basic_consume(queue = 'ppd/election/2' , on_message_callback = callback2, auto_ack = True)
    
    # Challenger
    channel.basic_consume(queue = 'ppd/challenge/2' , on_message_callback = callback3, auto_ack = True)
    
    # Seed
    channel.basic_consume(queue = 'ppd/seed/2' , on_message_callback = callback4, auto_ack = True)
    
    # Resultado
    channel.basic_consume(queue = 'ppd/result/2' , on_message_callback = callback5, auto_ack = True)
