...
def main():
    ...
    room = channel.queue_declare(queue = 'ppd/WRoom/4')      # assina/publica - Sala de Espera
    channel.queue_bind(exchange='WRoom', queue=room.method.queue)

    channel.exchange_declare(exchange='Election', exchange_type='fanout')
    election = channel.queue_declare(queue = 'ppd/election/4')      # assina/publica - Eleção do presidente
    ...
    challenge = channel.queue_declare(queue = 'ppd/challenge/4')     # assina/publica - Desafio da transição atual
    ...
    seed = channel.queue_declare(queue = 'ppd/seed/4')     # assina/publica - Verificação da seed que resolve desafio
    ...
    result = channel.queue_declare(queue = 'ppd/result/4')     # assina/publica - Lista de votação na seed que soluciona o desafio
    ...
    channel.basic_consume(queue = 'ppd/WRoom/4' , on_message_callback = callback, auto_ack = True)
    
    # Eleição
    channel.basic_consume(queue = 'ppd/election/4' , on_message_callback = callback2, auto_ack = True)
    
    # Challenger
    channel.basic_consume(queue = 'ppd/challenge/4' , on_message_callback = callback3, auto_ack = True)
    
    # Seed
    channel.basic_consume(queue = 'ppd/seed/4' , on_message_callback = callback4, auto_ack = True)
    
    # Resultado
    channel.basic_consume(queue = 'ppd/result/4' , on_message_callback = callback5, auto_ack = True)
