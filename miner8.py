...
def main():
    ...
    room = channel.queue_declare(queue = 'ppd/WRoom/8')      # assina/publica - Sala de Espera
    channel.queue_bind(exchange='WRoom', queue=room.method.queue)

    channel.exchange_declare(exchange='Election', exchange_type='fanout')
    election = channel.queue_declare(queue = 'ppd/election/8')      # assina/publica - Eleção do presidente
    ...
    channel.basic_consume(queue = 'ppd/WRoom/8' , on_message_callback = callback, auto_ack = True)
    
    # Eleição
    channel.basic_consume(queue = 'ppd/election/8' , on_message_callback = callback2, auto_ack = True)
    
    # Challenger
    channel.basic_consume(queue = 'ppd/challenge/8' , on_message_callback = callback3, auto_ack = True)
    
    # Seed
    channel.basic_consume(queue = 'ppd/seed/8' , on_message_callback = callback4, auto_ack = True)
    
    # Resultado
    channel.basic_consume(queue = 'ppd/result/8' , on_message_callback = callback5, auto_ack = True)
